...
def ltr_dimension_reduction(adata, n_components=3):
...
def ltr_seed_elastic_principal_graph(adata, n_clusters=50, max_n_clusters=200,modLenList=[], minNumInClusterForInitTree=10):
    """Seeding the initial elastic principal graph.
    ...
    sc = SpectralClustering(n_clusters=n_clusters, affinity='nearest_neighbors', n_neighbors=10,
                            eigen_solver='arpack', random_state=42).fit(input_data)
    cluster_labels = sc.labels_
    toPlotData = pd.DataFrame(input_data)
    toPlotData = pd.concat([toPlotData,
                            pd.Series(cluster_labels)], axis=1)
    toPlotData.columns = ['x', 'y', 'z', 'label']
    toPlotData = pd.DataFrame(input_data)
    toPlotData = pd.concat([toPlotData,
                            pd.Series(cluster_labels)], axis=1)
    toPlotData.columns = ['x', 'y', 'z', 'label']
    totSegData = pd.DataFrame(columns=['ax', 'ay', 'az', 'bx', 'by', 'bz'])


    Y=np.asmatrix(np.array(modLenList)).reshape(-1,1)
    X=np.asmatrix(toPlotData['x']).reshape(-1,1)
    testReg = LinearRegression()
    testReg.fit(X, Y)
    relaK = testReg.coef_[0][0]
    totXMin = np.min(toPlotData['x'])
    totXMax = np.max(toPlotData['x'])
    for nowCluster in pd.Series(cluster_labels).drop_duplicates().tolist():
        clustData = toPlotData[toPlotData['label'] == nowCluster]
        # default 10
        # for cotton 20
        if clustData.shape[0] < minNumInClusterForInitTree:
            continue
        X = clustData['x'].to_numpy()
        X = np.asmatrix(X, dtype='float32').reshape(-1, 1)
        Y = clustData[['y', 'z']].to_numpy()
        Y = np.asmatrix(Y, dtype='float32')
        reg = LinearRegression()
        reg.fit(X, Y)
        segPaX = clustData['x'].quantile(0.9)
        segPbX = clustData['x'].quantile(0.1)
        segX = np.asarray([segPaX, segPbX]).reshape(-1, 1)
        segY = reg.predict(segX)
        segY = pd.DataFrame(segY)
        segX = np.reshape(segX, 2)
        segData = pd.concat([pd.Series(segX),
                             segY], axis=1)
        segData.columns = ['x', 'y', 'z']
        tmpData = pd.DataFrame([segData.iloc[0, :].tolist() + segData.iloc[1, :].tolist()],
                               columns=['ax', 'ay', 'az', 'bx', 'by', 'bz'])
        totSegData = pd.concat([totSegData, tmpData], axis=0, ignore_index=True)
    ttg = nx.Graph()
    nodeIndex = 0
    for ind, row in totSegData.iterrows():
        ttg.add_node(nodeIndex,
                     aPos=(row['ax'], row['ay'], row['az']),
                     bPos=(row['bx'], row['by'], row['bz']))
        nodeIndex = nodeIndex + 1
    nodesNum = nodeIndex
    # 添加两两之间的边权重
    def calPandalPara(posA, posB):
        if relaK < -0.3:
            minerX = min(posA[0], posB[0])
            return (totXMax - minerX) / (totXMax - totXMin)
        elif relaK > 0.3:
            maxerX = max(posA[0], posB[0])
            return (maxerX - totXMin) / (totXMax - totXMin)
        else:
            return 1
    ...
    for i in range(nodesNum):
        for j in range(nodesNum):
            if i == j:
                continue
            _3dDis = np.linalg.norm(
                np.array([*(ttg.nodes[i]['aPos'])]) - np.array([*(ttg.nodes[j]['aPos'])])) * calPandalPara(
                ttg.nodes[i]['aPos'], ttg.nodes[j]['aPos'])
            _3dPp = (f'{i}_aPos', f'{j}_aPos')

            tmpDis = np.linalg.norm(
                np.array([*(ttg.nodes[i]['aPos'])]) - np.array([*(ttg.nodes[j]['bPos'])])) * calPandalPara(
                ttg.nodes[i]['aPos'], ttg.nodes[j]['bPos'])
            if tmpDis < _3dDis:
                _3dDis = tmpDis
                _3dPp = (f'{i}_aPos', f'{j}_bPos')

            tmpDis = np.linalg.norm(
                np.array([*(ttg.nodes[i]['bPos'])]) - np.array([*(ttg.nodes[j]['aPos'])])) * calPandalPara(
                ttg.nodes[i]['bPos'], ttg.nodes[j]['aPos'])
            if tmpDis < _3dDis:
                _3dDis = tmpDis
                _3dPp = (f'{i}_bPos', f'{j}_aPos')

            tmpDis = np.linalg.norm(
                np.array([*(ttg.nodes[i]['bPos'])]) - np.array([*(ttg.nodes[j]['bPos'])])) * calPandalPara(
                ttg.nodes[i]['bPos'], ttg.nodes[j]['bPos'])
            if tmpDis < _3dDis:
                _3dDis = tmpDis
                _3dPp = (f'{i}_bPos', f'{j}_bPos')

            ttg.add_edge(i, j, weight=_3dDis, specNode=_3dPp)

    ttMst = nx.minimum_spanning_tree(ttg)
    mstEdges = np.array(ttMst.edges())

    oriTreeNodeId2newId = defaultdict(int)
    ttInitTree = nx.Graph()
    initNodeId = 0

    for n in ttMst.nodes():
        posDict = ttMst.nodes[n]
        aposId = f'{n}_aPos'
        oriTreeNodeId2newId[aposId] = initNodeId
        ttInitTree.add_node(initNodeId, pos=posDict['aPos'])
        initNodeId = initNodeId + 1
        bposId = f'{n}_bPos'
        oriTreeNodeId2newId[bposId] = initNodeId
        ttInitTree.add_node(initNodeId, pos=posDict['bPos'])
        initNodeId = initNodeId + 1
    for e in np.array(ttMst.edges()):
        eDict = ttMst.edges[e[0], e[1]]
        if (eDict['specNode'][0] not in oriTreeNodeId2newId) or \
                (eDict['specNode'][1] not in oriTreeNodeId2newId):
            print(eDict['specNode'][0], 'or', eDict['specNode'][1], 'not in!', sep=' ')
        ttInitTree.add_edge(oriTreeNodeId2newId[eDict['specNode'][0]],
                            oriTreeNodeId2newId[eDict['specNode'][1]])
    for n in ttMst.nodes():
        aposId = f'{n}_aPos'
        bposId = f'{n}_bPos'
        ttInitTree.add_edge(oriTreeNodeId2newId[aposId],
                            oriTreeNodeId2newId[bposId])
    for n in ttInitTree.nodes():
        ttInitTree.nodes[n]['pos'] = np.array([*ttInitTree.nodes[n]['pos']])

    adata.obs['sc'] = ['cluster ' + str(x) for x in cluster_labels]
    ...
    dict_nodes_pos = dict()
    for n in epg.nodes():
        dict_nodes_pos[n] = epg.nodes[n]['pos']
    print('The number of initial nodes is ' + str(len(dict_nodes_pos)))
    dict_branches = extract_branches(epg)
    flat_tree = construct_flat_tree(dict_branches)
    nx.set_node_attributes(flat_tree, values={x: dict_nodes_pos[x] for x in flat_tree.nodes()}, name='pos')
    adata.uns['epg'] = deepcopy(epg)
