...
parser.add_argument('-t','--plotTimeInterval', type=float,nargs=2)
...
for br in kDict:
    if kDict[br] > 1:
        ...
        for i in range(len(cluster_labels)):
            ...
            labels_list.append(f'{br}__{int(cluster_labels[i])}')

        tmpDisMat.reset_index(drop=True,inplace=True)
        tmpDisMat = pd.concat([tmpDisMat,
                              pd.Series(labels_list)],
                              axis=1)
        tmpDisMat.columns = ['x','y','z','label']
        meanData = tmpDisMat.groupby('label').mean()
        chaData = meanData.max() - meanData.min()
        meanData.sort_values(by=chaData.idxmax(),inplace=True)
        plotOrder = meanData.index.to_list()
...
# Cancel plot insert time dynamics for each species
'''
for spe in speIdList:
    timeList = []
    conClassList = []
    modId2oldestInsTime = defaultdict(float)
    for oriId in oriId2insertTime:
        ...
        if spe in oriId and _3dClassList[oriId2modId[oriId]] != 'other':
            ...
            insTime = oriId2insertTime[oriId]
            if (modId not in modId2oldestInsTime) or \
               (modId2oldestInsTime[modId]<insTime):
                modId2oldestInsTime[modId] = insTime
    for modId in modId2oldestInsTime:
        timeList.append(modId2oldestInsTime[modId])
        conClassList.append(_3dClassList[modId])
    toPlotData = pd.concat([pd.Series(timeList),
                            pd.Series(conClassList)],
                           axis=1)
    toPlotData.columns = ['insertTime', 'typ']
    sns.boxplot(data=toPlotData, y='insertTime', x='typ', color='typ', palette = plotColorDict)
    plt.savefig(f'{figureD}/{args.outPre}.{spe}.pdf', bbox_inches='tight')
    plt.close()
'''
...
for oriId in oriId2insertTime:
    if len(args.lineage)>0 and (oriId2teClass[oriId] not in args.lineage):
        continue
    if _3dClassList[oriId2modId[oriId]] != 'other':
        modId = oriId2modId[oriId]
        insTime = oriId2insertTime[oriId]
        if (modId not in modId2oldestInsTime) or \
           (modId2oldestInsTime[modId]<insTime):
...
toPlotData = pd.concat([pd.Series(timeList),
                       ...
                       axis=1)
toPlotData.columns = ['insertTime', 'typ']
sns.boxplot(data=toPlotData, y='insertTime', x='typ', color='typ', palette = plotColorDict, order = plotOrder)
plt.xlabel('Cluster')
plt.ylabel('Insert time')
plt.xticks([])
plt.savefig(f'{figureD}/{args.outPre}.total.pdf', bbox_inches='tight')
plt.close()
